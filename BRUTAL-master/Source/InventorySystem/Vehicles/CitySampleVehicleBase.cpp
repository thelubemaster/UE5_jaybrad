...
void ACitySampleVehicleBase::BeginPlay()
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Failed to find FBodyInstance on vehicle, any center of mass adjustments won't be properly applied."));
	}
...
void ACitySampleVehicleBase::UpdateWheelFrictionMultiplierAdjustments()
{
	if (bUseWheelFrictionMultiplierAdjustments)
	{
		if (UChaosWheeledVehicleMovementComponent* const VMC = StaticCast<UChaosWheeledVehicleMovementComponent*>(GetVehicleMovement()))
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("Attempted to use WheelFrictionMultiplierAdjustments on a vehicle that doesn't have the proper wheels set up or with a missing WheelFrictionMultiplierCurve."))
			}
		}
	}
}
