...
{
	UE_LOG(LogTemp, Display, TEXT("Logging Custom Event %s to CSV."), *EventName);
...
void UCitySampleBlueprintLibrary::OverrideDeviceProfileForMode(EDeviceProfileOverrideMode NewMode)
{
	...
	{
		UE_LOG(LogTemp, Display, TEXT("%s is already active"), *GetActiveDeviceProfileName());
		return;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Cannot Find Device Profile Named %s"), *DeviceProfileName);
	}
}
