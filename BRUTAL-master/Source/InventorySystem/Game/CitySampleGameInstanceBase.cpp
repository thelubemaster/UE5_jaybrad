...
void UCitySampleGameInstanceBase::OnSaveGameComplete(const FString& SaveFile, const int32 UserIndex, bool bSuccess)
{
	if (!bSuccess)
	{
		UE_LOG(LogTemp, Warning, TEXT("Game was not successfully saved!"), *GetName());
	}
}
...
UCitySampleSaveGame* UCitySampleGameInstanceBase::LoadCitySampleSaveGame(bool* const bOutSaveGameFound/*=false*/)
{
	...
	else
	{
		...
		UE_LOG(LogTemp, Display, TEXT("%s: failed to load save data. Most likely no save data exists yet."), ANSI_TO_TCHAR(__func__));
		SaveGame = Cast<UCitySampleSaveGame>(UGameplayStatics::CreateSaveGameObject(UCitySampleSaveGame::StaticClass()));

		if (SaveGame == nullptr)
		{
			UE_LOG(LogTemp, Warning, TEXT("%s: failed to create a new save game."), ANSI_TO_TCHAR(__func__));
		}
	}
...
void UCitySampleGameInstanceBase::SaveCitySampleGameData(bool bAsync/*=true*/)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("No save data exists. Save failed..."));
...
void UCitySampleGameInstanceBase::ClearCitySampleSaveData()
{
	...
	if (UGameplayStatics::DoesSaveGameExist(SaveFileName, 0))
	{
		...
		SaveGame = nullptr;
		UE_LOG(LogTemp, Display, TEXT("CitySample Save Data has been cleared upon request."));
	}
	else
	{
		UE_LOG(LogTemp, Display, TEXT("Failed to clear save data. Most likely no save data exists yet."));
	}
}
