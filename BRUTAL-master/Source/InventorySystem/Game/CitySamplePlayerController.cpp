...
#include "CitySampleSaveGame.h"
#include "CitySampleControlsOverlay.h"
#include "CitySampleUIComponent.h"
...
void ACitySamplePlayerController::TryToInteract(UCitySampleInteractionComponent* Interaction)
{
	...
	if (IsInteracting())
	{
		UE_LOG(LogTemp, Warning, TEXT("%s::TryToInteract: Already interacting with %s"),
	...
	if (!Interaction)
	{
		UE_LOG(LogTemp,Warning, TEXT("%s::TryToInteract: Invalid interaction"),
			*GetPathName(this));
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("%s::TryToInteract: Interaction is busy %s"),
...
void ACitySamplePlayerController::TryToUpdateInteraction(UCitySampleInteractionComponent* ToInteractWith, uint8 InInteractionState)
{
	...
	if (!ToInteractWith)
	{
		UE_LOG(LogTemp, Error, TEXT("TryToUpdateInteraction_Validate:%s: Invalid Interaction Object. Object=%s, Type=%s"),
	...
	{
		UE_LOG(LogTemp, Error, TEXT("TryToUpdateInteraction_Validate:%s: Invalid Interaction Type. Object=%s, Type=%s"),
	...
	auto LogInteraction = [&]()
	{
		UE_LOG(LogTemp, Warning, TEXT("TryToUpdateInteraction:%s: Interaction. Object=%s, Type=%s, Reason=%s"),
...
void ACitySamplePlayerController::HandlePenetratingCameraTransition()
{
	...
	if (!World->GetTimerManager().IsTimerActive(CameraTransitionHidePenetrationHandle))
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("ACitySamplePlayerController::HandlePenetratingCameraTransition StartCameraFade to alpha 1 black (fade out)"));
...
void ACitySamplePlayerController::EndPenetratingCameraTransition()
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("ACitySamplePlayerController::CheckIfSafeToEndCameraTransition StartCameraFade to alpha 0 (fade in)"));
...
void ACitySamplePlayerController::HandleLongDistanceCameraTransition()
{
	...
	if (!World->GetTimerManager().IsTimerActive(CameraTransitionWaitingForLevelStreamingHandle))
	{
		if (PlayerCameraManager)
		{
			UE_LOG(LogTemp,Warning, TEXT("ACitySamplePlayerController::HandleLongDistanceCameraTransition StartCameraFade to alpha 1 black (fade out)"));
